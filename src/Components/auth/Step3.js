...
const Step3 = () => {
  ...
  const toggleCondition = (c) => {
    if (c === "None of the above") {
      ...
      setConditions((prev) =>
        prev.includes(c)
          ? prev.filter((x) => x !== c)
          : [...prev.filter((x) => x !== "None of the above"), c]
      );
    }
  };
  const handleContinue = () => {
    ...
    const medicalHistoryData = {
      ...
      surgeries: Array.isArray(formData.surgeries)
        ? formData.surgeries.join(", ")
        : formData.surgeries,
      conditions: Array.isArray(formData.conditions)
        ? formData.conditions.join(", ")
        : formData.conditions,
    };
    localStorage.setItem("step3Data", JSON.stringify(medicalHistoryData));
          ...
          >
            <div className="text-center">
              <h1 className="text-5xl font-bold mb-4">
                Medical <span className="text-[#4addbf]">History</span>
              </h1>
              <p className="text-white/70 text-lg">
                This helps us give you better care
              </p>
            </div>
                  ...
                  <button
                    ...
                    className={`py-4 rounded-2xl border transition-all ${
                      conditions.includes(c)
                        ? "bg-[#4addbf] text-black border-[#4addbf]"
            ...
            <div className="space-y-6">
              <textarea
                ...
                placeholder="Allergies (e.g. penicillin, nuts...)"
