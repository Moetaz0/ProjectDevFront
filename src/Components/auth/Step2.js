...
const Step2 = () => {
  ...
  const handleContinue = () => {
    if (canContinue) {
      const formData = {
        notesDaySingIn: mainConcern,
        dateOfSignIn: getDateFromLabel(whenStarted).toISOString(),
        ratedHealth: severity,
      };
  ...
  const getSliderStyle = () => {
    ...
    return {
      background: `linear-gradient(to right, #ef4444 0%, #f59e0b ${percent}%, #4addbf ${percent}%, #4addbf 100%)`,
    };
  };
          ...
          >
            <div className="text-center">
              <h1 className="text-5xl font-bold mb-4">
                ...
                <span className="text-[#4addbf]">Today?</span>
              </h1>
              <p className="text-white/70 text-lg">
                Tell us how you're feeling — be as specific as you can
              </p>
            </div>
            ...
            <div className="relative group">
              ...
              {mainConcern && (
                <FiCheckCircle
                  size={28}
                  className="absolute right-4 top-4 text-[#4addbf]"
                />
              )}
            </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                {[
                  ...
                  <button
                    key={opt}
                    type="button"
                    onClick={() => setWhenStarted(opt)}
                    className={`py-4 rounded-2xl border transition-all ${
                      whenStarted === opt
                        ? "bg-[#4addbf] text-black border-[#4addbf] shadow-lg"
            ...
            <div className="space-y-6">
              ...
              <input
                ...
                className="w-full h-3 bg-white/20 rounded-full appearance-none cursor-pointer accent-[#4addbf]"
                style={getSliderStyle()}
              />
              <div className="flex justify-between text-sm text-white/70">
                <span>1 - Barely noticeable</span>
                <span className="text-2xl font-bold text-[#4addbf]">
                  {severity}/10
                </span>
            ...
            <div className="flex justify-between pt-8">
              <button
                onClick={() => navigate("/SignUp")}
                className="px-8 py-4 rounded-2xl bg-white/10 hover:bg-white/20 border border-white/20 flex items-center gap-3 transition"
              >
              ...
              <button
                ...
                className={`px-10 py-4 rounded-2xl font-bold flex items-center gap-3 transition-all shadow-[0_0_40px_#4addbf70] ${
                  canContinue
                    ? "bg-[#4addbf] hover:bg-[#39c6a5] text-black"
